...
1. **`BackendEngine` (ABC)** â€” defines the contract for all 55+ tool methods
2. **`PandasEngine`** â€” concrete implementation using pandas/numpy
3. **`SparkEngine`** â€” concrete implementation using PySpark SQL + Vectorized UDFs
...
6. A **context manager** (`etlpipe.backend("spark")`) enables scoped backend switching

The Spark engine uses a 3-tier execution strategy:
- **Tier 1**: Native Spark SQL (preferred â€” fully distributed)
...
- Users write pipeline logic exactly once â€” same code runs on laptop or 100-node cluster
- Thread-safe â€” multiple backends can run concurrently in the same process
